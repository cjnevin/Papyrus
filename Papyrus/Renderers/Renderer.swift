...
protocol Renderer {
    ...
    func drawPath(_ path: CGPath, color: UIColor, lineColor: UIColor, lineWidth: CGFloat, rect: CGRect)
}


extension Renderer {
    /// Fills a rectangle using a `shader` to provide `fillColor`.
    func fillRect(_ rect: CGRect, shader: Shader) {
        fillRect(rect, color: shader.fillColor!)
    }
    
    /// Strokes a rectangle using `shader` to provide `strokeColor` and `strokeWidth`.
    func strokeRect(_ rect: CGRect, shader: Shader) {
